# See https://github.com/Bin4ry/free-dog-sdk on how to use
...
VelStopF  = 16000.0
dt = 0.002
TORQUE_RANGE = [-4.0, 4.0] # Change these for a bigger torque range
sin_count = TORQUE_RANGE[0]
...
while True:
    ...
    if( motiontime >= 500):
        sin_count += 0.0001
        #torque = (0 - lstate.motorState[d['FL_1']].q)*10.0 + (0 - lstate.motorState[d['FL_1']].dq)*1.0
        if (sin_count <= TORQUE_RANGE[1]):
            torque = sin_count
        else:
            torque = 0.0
            #torque = np.sin(sin_count)/1.1
        torque = np.fmin(np.fmax(torque, -5.0), 5.0)
       
        #print(torque)
        #                    Change this to sweep a different actuator
        mCmdArr.setMotorCmd('RL_2',  motorCmd(mode=MotorModeLow.Servo, q=PosStopF, dq = VelStopF, Kp = 0, Kd = 4, tau = torque))
        lcmd.motorCmd = mCmdArr
        print(motorCmd(mode=MotorModeLow.Servo, q=PosStopF, dq = VelStopF, Kp = 0, Kd = 4, tau = torque).getBytes())
